...
def getLikeCount(page_name):
    ...
    page_text = page_text[page_text.rfind(UPPER_DELIMITER_0)
    + len(UPPER_DELIMITER_0):]
    """
    ...
    page_text = page_text[page_text.rfind(UPPER_DELIMITER_0) + len(
        UPPER_DELIMITER_0):].replace(SPACE, '').replace(' ', '')
    return page_text if page_text == str(int(page_text))\
        else "Error: check page name"
